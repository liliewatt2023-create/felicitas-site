...
export async function sendWelcomeEmail({
  ...
  try {
    await resend.emails.send({
      from: "Charcuterie Felicita <onboarding@resend.dev>", // Utilise le domaine de test Resend
      to,
    ...
    console.log(`✅ Email de bienvenue envoyé à ${to} via Resend`);
    return { success: true };
  } catch (error) {
...
export async function sendReviewModerationEmail(
  ...
  try {
    ...
    console.log(`✅ Email de modération envoyé via Resend`);
    return { success: true };
  } catch (error) {
